...
6,542 real, individually-verified Scrabble tile crops, organized as an `ImageFolder`-compatible
directory (`<LETTER>/*.png`, plus `BLANK/` for blank tiles) for `training/classify/train.py`.
2,836 are board-crop tiles (60x60 canonical, one per occupied board cell); the remaining 3,706 are
...
| `wespa_game3_final` | WESPA Word Wars Game 3, same broadcast/event | 99 | `tests/fixtures/wespa_word_wars_game3.gcg`, `board_from_gcg_final` from a single direct-frame screenshot (t=10310s) extracted via `yt-dlp -f 399 -g` + `ffmpeg` seek, no video download; final score 468/510 verified against the on-screen graphic |
| `wespa_game4_final` | WESPA Word Wars Game 4, same broadcast/event | 97 | `tests/fixtures/wespa_word_wars_game4.gcg`, same single-frame technique (t=15750s); final score 435/422 verified |
| `wespa_game5_final` | WESPA Word Wars Game 5, same broadcast/event | 92 (4 dropped) | `tests/fixtures/wespa_word_wars_game5.gcg`, same technique (t=17850s); final score 364/510 verified |
| `wespa_game6_final` | WESPA Word Wars Game 6, same broadcast/event | 95 (1 dropped) | `tests/fixtures/wespa_word_wars_game6.gcg`, same technique (t=20780s); final score 509/463, identity confirmed via the "YaUP" ticker move against the GCG (an earlier draft mis-picked this frame as Game 5 until the ticker text was grepped against each game's GCG) |
| `wespa_game7_final` | WESPA Word Wars Game 7, same broadcast/event | 94 (4 dropped) | `tests/fixtures/wespa_word_wars_game7.gcg`, same technique (t=23860s); final score Nigel Richards 529 / Adam Logan 422, confirmed via the "14C RUED +21 525" ticker move — an earlier candidate frame showed "RUED" in what looked like a hand-drawn annotation overlay rather than a genuine tile, resolved by advancing to the frame where the tile font and the score both cleanly matched the GCG's true final state |
...
**Games 3-7 (per explicit request: "go through a bunch of games... just take screenshots... rather than downloading"), 477 tiles after spot-check, finally give real rare-letter coverage for this venue**: J 55, K 59, Q 60, X 61, Z 66 (up from J:2, K:2, Q:0, Z:0 after games 1-2). Located game boundaries in the single long "Games 1-7" video via downloaded storyboard sprite sheets (`yt-dlp -f sb1`, no full video download), then pulled one direct HD frame per game (`yt-dlp -f 399 -g` for a signed googlevideo URL, `ffmpeg -ss <t> -frames:v 1`) once each candidate final frame's on-screen score and ticker text were grep-verified against that game's own GCG -- necessary discipline after one early frame was nearly mislabeled as the wrong game (see `wespa_game6_final` above).

**Spot-check montages (one crop per class, this project's mandatory pre-promotion gate) caught real corruption concentrated entirely in board row 14** -- the bottom board row sits right at the video frame's edge in this camera framing, and every affected crop showed either a neighboring cell's content bleeding in with none of the true label visible, or a blank/blurred gradient with no tile at all: `wespa_game5_final` (4 of its 12 row-14 crops: E, I x2, M), `wespa_game6_final` (1 of 2: a "B"-labeled crop showing a plain "R" premium-square tile with no B anywhere), and `wespa_game7_final` (all 4 of its row-14 crops -- this frame's bottom row is almost entirely outside the visible board in the rectified image). All 9 were individually confirmed bad by eye (not just suspected) and dropped before promoting; games 3 and 4's row-14 crops were individually checked too and found clean.

**A real, if modest, regression at the standard recipe -- fixed by halving the learning rate, not by discarding the data.** The usual gentle continuation (2 epochs, `lr=1e-4`, from the `wespa_game2_final` checkpoint) on these 477 tiles cost 2 points on the Causeway held-out set (97.5% [118/121] -> 95.9% [116/121]): two new mistakes on previously-correct, unambiguous tiles (G->B twice, L->J once -- verified by eye, not sample-size noise on an easy class). Cutting to 1 epoch at the same `lr` didn't help (still 95.9%, identical new mistakes) -- ruling out epoch count as the cause. Halving the learning rate instead (`lr=5e-5`, still 2 epochs) fully restored zero regression: 97.5% (118/121), the identical 3 baseline mistakes (BLANK->X x2, Q->C). This is the first time this project's "2 epochs / 1e-4" default has needed adjusting -- worth trying `5e-5` first on any future round this size (~450+ new tiles at once) rather than assuming the standard rate is always safe. Recalibrated (`T≈1.074`); promoted after presenting both the regression and the fix to the user.
